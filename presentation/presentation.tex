...
\tikzset{
  ...
  demo/.pic={
    ...
    \draw[wire] (x) to (not.input);
    \draw[wire] (not.output) to (or.input 1);
    \draw[wire] (y) to (y') to (or.input 2);
    \draw[wire] (or.output) to (out);
  },
}
...
\begin{frame}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \draw[wire] (x) to (x') to (o1.input 1) (x) to (nx.input);
