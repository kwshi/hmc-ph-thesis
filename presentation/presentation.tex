...
\tikzset{
  ...
  pics/csat/.style n args={3}{
    code={
      ...
      \pic at (gates.south east) {gate legend};
...
\begin{frame}
  ...
  \begin{center}
    \begin{tikzpicture}
      \alt<3->{\pic{csat=100}}{\pic{csat=000}};
    \end{tikzpicture}

    \visible<2->{Is there an input combination that \ul{satisfies} this
    circuit?} \visible<3->{Yes!}
  ...
  \visible<4->{
    ...
    \begin{itemize}[nosep]
      \item Circuits can represent \emph{any} function \(\{0,1\}^m \to
        \{0,1\}^n\)
      \item CSAT is \ul{$\NP$-complete} (a ``maximally hard'' problem in
        \(\NP\))
        \begin{itemize}[nosep]
          \item Aside: \(\NP \cong \text{problems solvable by guess-and-check}\)
        \end{itemize}
    \end{itemize}
  }
...
\begin{frame}
  \frametitle{The two-turn \(\CSAT\) \emph{game}}
  \begin{center}
    \begin{tikzpicture}
...
\begin{frame}
  ...
  \begin{itemize}
    \item \(\CSAT_1 = \CSAT\):\quad \(\exists X \;
      \operatorname{circuit}(X) = 1\)?
    \item \(\CSAT_2\):\quad \(\exists X_1 \; \forall X_2 \;
      \operatorname{circuit}(X\dots) = 1\)?
    \item \(\CSAT_3\):\quad \(\exists X_1 \; \forall X_2 \; \exists X_3 \;
      \operatorname{circuit}(X\dots) = 1\)?
    \item[] \(\vdotswithin{\CSAT_n}\)
  \end{itemize}

  \pause
  \[
    \underbrace{\CSAT_1}_{\text{\(\NP\)-complete}}
    \le \underbrace{\CSAT_2}_{\text{\(\Sigma_2\P\)-complete}}
    \le \underbrace{\CSAT_3}_{\text{\(\Sigma_3\P\)-complete}}
    {} \le \dotsb
  \]
...
\begin{frame}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \matrix[column sep=4em]{
        \pic{uncolorable}; \& \pic{colorable \step}; \\
      };
    \end{tikzpicture}
  ...
  \visible<3->{
    Q: How difficult is \(3\COL\)?
    \begin{itemize}[nosep]
      \item Fact: \(3\COL \in \NP\), so \(3\COL \le \CSAT\).
