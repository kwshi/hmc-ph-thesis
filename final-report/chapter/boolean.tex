...

{
  ...
  \tikzset{
    ...
    wires/.pic={
      ...
      \foreach \var in {x,y,z} {
        ...
        \draw[pipe] (\var1) node[left]{\(\var₁\)} to (\var.west);
        \draw[pipe] (\var2) node[left]{\(\var₂\)} to (\var.west);
        \node[above right] at (\var.east) {\(\MakeUppercase{\var}=\var₁∨\var₂\)};
      }
      \draw[pipe] (x.east) to (xy.west);
      \draw[pipe] (y.east) to (xy.west);
      \draw[pipe] (y.east) to (yz.west);
      \draw[pipe] (z.east) to (yz.west);
      \draw[pipe] (z.east) to (zx.west);
      \draw[pipe, over] (x.east) to (zx.west);
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \pic{wires};
      \foreach \gate in {xy,yz,zx} {
        \draw[pipe] (\gate.east) to (out.west);
      }
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \pic{wires};
      \draw[pipe] (xy.east) to (or.west);
      \draw[pipe] (yz.east) to (or.west);
      \draw[pipe] (or.east) to (or'.west);
      \draw[pipe] (zx.east) to (or'.west);
...
% TODO blurb about circuits being universal in computers

% TODO precise definitions, gate symbols, etc.
