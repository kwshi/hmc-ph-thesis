...
TODO diagram of an example conversion for 3x3 or 2x2
...
same yes/no answer.  We formalize this idea as follows:
...
\begin{definition}{reductions}{}
  ...
  \begin{aside}
    TODO remark this is many-one/karp reduction, not to be confused with
  \end{aside}

\end{definition}

In the above example, the reduction from \Problem{Latin Square} to
\Problem{Graph Coloring} captures the idea that \Problem{Latin Square} is
easier than \Problem{Graph Coloring} in the following sense.  Suppose that we
already know how to solve \Problem{Graph Coloring}.  Then, we automatically
also know how to solve \Problem{Latin Square}: given an arbitrary
...
However, in order to honestly capture the idea of \emph{easiness}, we must also
account for computation time.  Namely, we stipulate that the reduction itself
must be efficient---formally, that it runs in polynomial time.
