...
The inductive definition of this game is given below.
\begin{enumerate}
...
\begin{problem}{\Problem{3-Colorability} with \(k\) turns / \Col[k]}{}
  ...
  TODO finish, and also i think the recursive defn below is not exactly
  right---I think the check-that-each-proper-coloring requirement needs to
  happen in the subproblem in order to fit this defn to sigmap recursion

  \tcblower
  \Col[k] = \SetBuilderLong{(Γ,κ,(U₁,\dotsc,Uₖ))}{
    ∃δ\colon U₁→\Colors\Q
    \text{\(κ[δ]\) is proper and \((Γ,κ[δ],(U₂,\dotsc,Uₖ))∉\Col[k-1]\)}
  }
\end{problem}
